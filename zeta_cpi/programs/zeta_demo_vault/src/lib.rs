...
pub mod zeta_cpi {
    ...
    pub fn read_program_data(ctx: Context<ReadProgramData>) -> ProgramResult {
        ...
        msg!(
            "Market strike: {:?}",
            zeta_group.get_strike(front_expiry_index)
        );
        msg!(
            "Market expiry: {:?}",
            zeta_group.expiry_series[front_expiry_index].expiry_ts
        );
        msg!(
            "Is market expired?: {:?}",
            zeta_group.expiry_series[front_expiry_index]
                .status()
                .unwrap()
                == ExpirySeriesStatus::Expired
        );
        ...
        msg!(
            "Delta: {:?}",
            greeks.product_greeks[front_expiry_index].delta
        );
        msg!(
            "Vega: {:?}",
            Decimal::from(greeks.product_greeks[front_expiry_index].vega)
        );
        msg!(
            "IV: {:?}",
            Decimal::from(greeks.product_greeks[front_expiry_index].volatility)
        );
        ...
        msg!(
            "Market net position: {:?}",
            margin_account.positions[front_expiry_index].position
        );
        msg!(
